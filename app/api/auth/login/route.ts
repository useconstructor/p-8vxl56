...
export async function POST(req: Request) {
  ...
  if (!user && email === process.env.ADMIN_EMAIL) {
    ...
    await db.execute({ sql: "INSERT INTO users (email,name,password_hash) VALUES (?,?,?)", args: [email, "Admin", hash] });
  ...
  const token = await new SignJWT({ email, name: user.name, isAdmin }).setProtectedHeader({ alg: "HS256" }).setExpirationTime("7d").sign(secret);
  const headers = new Headers({ "Content-Type": "application/json" });
  headers.append("Set-Cookie", `session=${token}; HttpOnly; Path=/; SameSite=Lax; Max-Age=604800; ${process.env.NODE_ENV === "production" ? "Secure" : ""}`);
  return new Response(JSON.stringify({ ok: true, email, isAdmin }), { headers });
}
